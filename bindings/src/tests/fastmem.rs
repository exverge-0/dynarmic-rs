mod a32 {
    ...
    const _: () = assert!(std::mem::offset_of!(ArmFastmemTestEnv, base) == 0);

    impl ArmFastmemTestEnv {
        fn new(backing_memory: *mut u8) -> Self {
            Self {
                base: UserCallbacks::new(&ArmFastmemTestEnv_CALLBACKS),
                ticks_left: 0,
                backing_memory
            }
        }
    ...
    unsafe extern "C" fn memory_read_code(env: *mut UserCallbacks, vaddr: VAddr) -> cpp_optional<u32> {
        let env = unsafe { &*env.cast::<ArmFastmemTestEnv>() };
    ...
    unsafe extern "C" fn memory_write_8(env: *mut UserCallbacks, vaddr: VAddr, val: u8) {
        let env = unsafe { &mut *env.cast::<ArmFastmemTestEnv>() };
        env.write(vaddr, val);
    }
    ...
    static ArmFastmemTestEnv_CALLBACKS: UserCallbacksVTable = UserCallbacksVTable::new(
    ...
    fn a32_fastmem() {
        unsafe {
            ...
            let mut config = UserConfig::new(&mut env.base, None);
            ...
            *ptr.add(8).cast::<u32>() = 0xEAFFFFFE; // B .
            ...
            assert_eq!(std::slice::from_raw_parts(ptr.add(0x100), 4).cmp(std::slice::from_raw_parts(ptr.add(0x1F0), 4)), Ordering::Equal);
        }
    }
}
mod a64 {
